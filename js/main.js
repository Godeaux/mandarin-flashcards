    ...
    if (catToggle) {
      catToggle.addEventListener("click", () => {
        ...
        if (filters) {
          ...
          catToggle.textContent = filters.classList.contains("cat-collapsed") ? "Filter ▾" : "Filter ▴";
        }
      });
    }
  ...
  initSwipeHandler() {
    ...
    const onEnd = () => {
      ...
      if (Math.max(absDx, absDy) >= THRESHOLD) {
        ...
        if (absDx > absDy) {
          if (dx < 0) { exitClass = 'exit-left'; rating = 0; }
          else { exitClass = 'exit-right'; rating = 3; }
        } else {
          if (dy < 0) { exitClass = 'exit-up'; rating = 2; }
          else { exitClass = 'exit-down'; rating = 1; }
        }
