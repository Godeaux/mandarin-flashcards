#!/usr/bin/env python3
"""Generate 4 audio variants per card with different speaking styles.

Each variant uses a different system instruction for Qwen3-TTS.
Output: audio/variants/<char>_v1.mp3 through <char>_v4.mp3

Usage:
  python generate_audio_v3.py              # all cards
  python generate_audio_v3.py --chars 山水火  # specific characters only
"""
import argparse
import os
import numpy as np
...
# 4 variant styles — different system instructions
VARIANT_STYLES = [
    "Speak naturally and clearly in Mandarin Chinese.",
    "Speak slowly and precisely, emphasizing each tone carefully, as if teaching a beginner.",
    "Speak at a moderate pace with natural conversational flow.",
    "Speak clearly with slightly exaggerated tones for learning purposes.",
]

# All 31 characters with example sentences (same as v2)
CARDS = [
    ("好", "hǎo", "你好！",
     "you. 你. nǐ. good. 好. hǎo"),
    ("中", "zhōng", "我来中国了。",
...
def main():
    parser = argparse.ArgumentParser(description="Generate 4 audio variants per flashcard")
    parser.add_argument("--chars", type=str, default="",
                        help="Specific characters to generate (e.g. '山水火'). Default: all")
    args = parser.parse_args()

    # Filter cards if --chars specified
    cards = CARDS
    if args.chars:
        char_set = set(args.chars)
        cards = [c for c in CARDS if c[0] in char_set]
        if not cards:
            print(f"No matching cards for: {args.chars}")
            return

    out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "audio", "variants")
    ...
    for char, pinyin, chinese, breakdown in cards:
        # Bookend format: Chinese sentence → breakdown → Chinese sentence
        text = f"{chinese} {breakdown}. {chinese}"

        for vi, style in enumerate(VARIANT_STYLES, 1):
            mp3_path = os.path.join(out_dir, f"{char}_v{vi}.mp3")
            wav_path = f"/tmp/tts_variant_{char}_v{vi}.wav"

            print(f"\nGenerating: {char} v{vi} ({pinyin})", flush=True)
    ...
    print(f"\n{'='*40}")
    print(f"Done! {success}/{total} variants generated.", flush=True)
    if failed:
